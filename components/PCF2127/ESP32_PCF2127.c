 ...
 *  @param  len
 */
static void pcf_i2c_read(ESP32_PCF2127* PCF, uint8_t reg, uint8_t* buffer, uint8_t len) {
    uint8_t reg_buffer[1] = {reg};
    ESP_ERROR_CHECK(i2c_master_write_to_device(PCF->i2c_port, PCF2127_I2C_ADDRESS, reg_buffer, 1, 1000 / portTICK_PERIOD_MS));
    ESP_ERROR_CHECK(i2c_master_read_from_device(PCF->i2c_port, PCF2127_I2C_ADDRESS, buffer, len, 1000 / portTICK_PERIOD_MS));
}
 ...
 *
 */
static void pcf_i2c_write8(ESP32_PCF2127* PCF, uint8_t reg, uint8_t data) {
    uint8_t buffer[2] = {reg, data};
    ESP_ERROR_CHECK(i2c_master_write_to_device(PCF->i2c_port, PCF2127_I2C_ADDRESS, buffer, 2, 1000 / portTICK_PERIOD_MS));
}
 ...
 *
 */
static void pcf_i2c_write40(ESP32_PCF2127* PCF, uint8_t reg, uint8_t* data) {
    uint8_t buffer[6] = {reg, data[0], data[1], data[2], data[3], data[4]};
    ESP_ERROR_CHECK(i2c_master_write_to_device(PCF->i2c_port, PCF2127_I2C_ADDRESS, buffer, 6, 1000 / portTICK_PERIOD_MS));
}
 ...
 *
 */
static void pcf_i2c_write56(ESP32_PCF2127* PCF, uint8_t reg, uint8_t* data) {
    uint8_t buffer[8] = {reg, data[0], data[1], data[2], data[3], data[4], data[5], data[6]};
    ESP_ERROR_CHECK(i2c_master_write_to_device(PCF->i2c_port, PCF2127_I2C_ADDRESS, buffer, 8, 1000 / portTICK_PERIOD_MS));
}
 ...
 */
static void pcf_i2c_writeBits(ESP32_PCF2127* PCF, uint8_t reg, uint8_t mask, uint8_t value) {
    uint8_t tmp[1];
    pcf_i2c_read(PCF, reg, tmp, 1);
    tmp[0] &= ~mask;
    tmp[0] |= (value & mask);
    pcf_i2c_write8(PCF, reg, tmp[0]);
}
...
void PCF_init(ESP32_PCF2127* PCF, uint8_t i2c_port){
    ...
    assert(PCF->init!=true && ALREADY_INIT);
    //Load default values
    PCF->i2c_port = i2c_port;
...
{   
    assert(PCF->init==true && NOT_INIT);
	uint8_t buf[9];
	/*
	 * Avoid reading CTRL2 register as it causes WD_VAL register
	 * value to reset to 0 which means watchdog is stopped.
	 */
	pcf_i2c_read(PCF, PCF2127_REG_CTRL3, buf, 9);
	...
	PCF->time.tm_sec = bcd2bin(buf[1] & 0x7F);      /* rtc sec 0-59 */
	PCF->time.tm_min = bcd2bin(buf[2] & 0x7F);      /* rtc min 0-59 */
	PCF->time.tm_hour = bcd2bin(buf[3] & 0x3F);     /* rtc hr 0-23 */
	PCF->time.tm_mday = bcd2bin(buf[4] & 0x3F);     /* rtc day of month 1-31 */
    PCF->time.tm_wday = buf[5] & 0x07;              /* rtc day of week 0-6 */
    PCF->time.tm_mon = bcd2bin(buf[6] & 0x1F) - 1;  /* rtc month 1-12 */
	PCF->time.tm_year = bcd2bin(buf[7]) + 100;      /* rtc year 0-99 */
...
{   
	...
	buf[0] = bin2bcd(time->tm_sec);	/* this will also clear OSF flag */
	buf[1] = bin2bcd(time->tm_min);
	buf[2] = bin2bcd(time->tm_hour);
	buf[3] = bin2bcd(time->tm_mday);
    buf[4] = time->tm_wday & 0x07;
    ...
    pcf_i2c_write56(PCF, PCF2127_REG_SC, buf);
...
{   
	...
	pcf_i2c_read(PCF, PCF2127_REG_CTRL2, &ctrl2, 1);

    // TODO: ? See todo in the top of the file
	// pcf2127_wdt_active_ping(&pcf2127->wdd);

	pcf_i2c_read(PCF, PCF2127_REG_ALARM_SC, buf, 5);

	PCF->alarm.enabled = !!(ctrl2 & PCF2127_BIT_CTRL2_AIE);
	PCF->alarm.pending = !!(ctrl2 & PCF2127_BIT_CTRL2_AF);

	PCF->alarm.tm.tm_sec = bcd2bin(buf[0] & 0x7F);
	PCF->alarm.tm.tm_min = bcd2bin(buf[1] & 0x7F);
	PCF->alarm.tm.tm_hour = bcd2bin(buf[2] & 0x3F);
	PCF->alarm.tm.tm_mday = bcd2bin(buf[3] & 0x3F);
    PCF->alarm.tm.tm_wday = buf[4] & 0x07;
    return &PCF->alarm;
}
...
{   
    assert(PCF->init==true && NOT_INIT);
    pcf_i2c_writeBits(PCF, PCF2127_REG_CTRL2, PCF2127_BIT_CTRL2_AIE, (uint8_t)enable << 1);
...
void PCF_rtc_set_alarm(ESP32_PCF2127* PCF, struct alm* alarm)
{
    ...
    pcf_i2c_writeBits(PCF, PCF2127_REG_CTRL2, PCF2127_BIT_CTRL2_AF, 0x00);
    ...
    * also weekdays alarm
    */
	buf[0] = bin2bcd(alarm->tm.tm_sec);
	buf[1] = bin2bcd(alarm->tm.tm_min);
	buf[2] = bin2bcd(alarm->tm.tm_hour);
	buf[3] = bin2bcd(alarm->tm.tm_mday);
    buf[4] = alarm->tm.tm_wday;

	pcf_i2c_write40(PCF, PCF2127_REG_ALARM_SC, buf);
    alarm->enabled = true;
    PCF_rtc_alarm_irq_enable(PCF, alarm->enabled);
}
